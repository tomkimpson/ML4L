...
def plot_matches(df,q):
    ...
    #Create geopandas
    geometry = [Point(xy) for xy in zip(df['longitude_ERA'], df['latitude_ERA'])]
    gdf = GeoDataFrame(df, geometry=geometry)   
...
def pipeline(date,utc_hour,satellite,latitude_bound,ERA_fields,tolerance):
    ...
    ERA = get_era_data(date, utc_hour, field=ERA_fields,bounds=bounds)
...
IO_path = '/network/group/aopp/predict/TIP016_PAXTON_RPSPEEDY/ML4L/ECMWF_files/raw/outputs/'
